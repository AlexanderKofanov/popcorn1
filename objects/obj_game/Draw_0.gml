...
// Стол отдельным спрайтом
var table_w = table_right - table_left;
var table_h = table_bottom - table_top;
draw_sprite_stretched(spr_table, 0, table_left, table_top, table_w, table_h);

// Общий список объектов на столе (сортировка depth = -y)
var draw_items = [];
...
for (var d = 0; d < array_length(draw_items); d++)
{
	...
	switch (item.kind)
	{
		case 0:
			...
			var shadow_a = 0.28 - min(0.2, k.z * 0.008);
			draw_set_alpha(max(0.04, shadow_a));
			draw_sprite_ext(spr_seed, 0, k.x + 1, k.y + 2, 0.16, 0.1, 0, c_black, 1);
			draw_set_alpha(1);
			draw_sprite_ext(spr_seed, 0, k.x, k.y - k.z, 0.2, 0.2, k.image_angle, c_white, 1);
		break;

		case 1:
			var p = popcorns[item.idx];
			var burst_k = 1 + p.burst_timer / 20;
			var alpha = p.collected ? p.collect_timer / 16 : 1;
			draw_set_alpha(alpha * 0.28);
			draw_sprite_ext(spr_popcorn, 0, p.x + 2, p.y + 2, 0.34 * burst_k, 0.22 * burst_k, 0, c_black, 1);
			draw_set_alpha(alpha);
			draw_sprite_ext(spr_popcorn, 0, p.x, p.y - p.z, 0.45 * burst_k, 0.45 * burst_k, p.image_angle, c_white, 1);
			draw_set_alpha(1);
		break;

		case 2:
			draw_set_alpha(0.22);
			draw_sprite_ext(spr_candle, 0, candle_x + 2, candle_y + 4, 0.46, 0.24, 0, c_black, 1);
			draw_set_alpha(1);
			draw_sprite_ext(spr_candle, 0, candle_x, candle_y, 0.48, 0.48, 0, c_white, 1);
...
draw_text(36, 30, "$ " + string(score));
draw_text(36, 74, "Перетаскивай свечку. Нажимай на попкорн для +$1");
